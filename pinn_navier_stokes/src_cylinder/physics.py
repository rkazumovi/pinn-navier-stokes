...
(flow past a cylinder, vortex shedding regime).
...
Same autograd machinery as the steady cavity-flow case, plus first-order
time derivatives u_t, v_t. No second time-derivative is needed since
Navier-Stokes is first-order in time.
"""
...
if __name__ == "__main__":
    ...
    # --- now test with the actual model + real sampling ---
    from model import CylinderPINN
    from sampling import sample_interior_points

    model = CylinderPINN(hidden_layers=4, hidden_units=20)
