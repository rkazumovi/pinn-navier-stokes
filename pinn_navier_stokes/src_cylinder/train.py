"""
Training loop for 2D unsteady flow past a cylinder (vortex shedding).

Total loss = PDE residual + initial condition + cylinder no-slip
  ...
  L_cyl     = MSE(u_pred) + MSE(v_pred)                       on cylinder surface (no-slip)
...
W_CYL = 5.0       # weighted higher: no-slip on the obstacle is critical for wake formation
W_INLET = 1.0
W_FARFIELD = 1.0
W_OUTLET = 1.0
...
def compute_cylinder_loss(model, x_c, y_c, t_c, u_c, v_c):
...
def resample_all():
    ...
    x_c, y_c, t_c, u_c, v_c = sample_cylinder_boundary(N_CYLINDER, device=DEVICE)
...
def train():
    ...
    model = CylinderPINN(hidden_layers=8, hidden_units=100).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        loss_cyl = compute_cylinder_loss(model, *pts["cylinder"])
        ...
        if epoch % 500 == 0 or epoch == 1:
            print(
                ...
                f"| cyl {loss_cyl.item():.6f} | inlet {loss_inlet.item():.6f} "
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "../outputs/pinn_cylinder.pt")
    print("\nModel saved to ../outputs/pinn_cylinder.pt")